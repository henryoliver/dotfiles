...
map('n', '<ESC>', '<Cmd>nohlsearch<Bar><Cmd>echo<CR>', default_options)

-- Move selected line / block of text in visual mode
map('x', 'K', "<Cmd>move '<-2<CR>gv-gv", default_options)
map('x', 'J', "<Cmd>move '>+1<CR>gv-gv", default_options)

-- Sort lines
map('x', '<Leader>la', "<Cmd>'<,'>sort<CR>", default_options)
map('x', '<Leader>ll', '<Cmd>! awk \'{ print length(), $0 | "sort -n | cut -d\\  -f2-" }\'<CR>', default_options)
...
whichkey.register({
    ...
    ['<Leader>vr'] = { '<Cmd>Reload<CR>', 'Reload' },
    ['<Leader>vR'] = { '<Cmd>Restart<CR>', 'Restart' },
})
...
whichkey.register({
    ...
    ['<Leader>cc'] = { "<Cmd>lua require('Comment.api').toggle_current_linewise()<CR>", 'Comment current line' },
    ['<Leader>cb'] = {
        "<Cmd>lua require('Comment.api').toggle_current_blockwise()<CR>",
        'Comment current line blockwise',
    },
})
